...
export default function QuizStats({
    ...
    if (isSpeedrun) {
        ...
        if (gameStartTime) {
            elapsedSeconds = Math.max(0, Math.floor((Date.now() - gameStartTime) / 1000));
        }
    ...
    }, [externalTimer, timer.isRunning, timer.seconds, timer.minutes, totalSeconds, elapsedSeconds, isSpeedrun]);
    ...
    const isLowTime = !isUnlimited && !isSpeedrun && totalSeconds <= 10 && totalSeconds > 0;
    const isCriticalTime = !isUnlimited && !isSpeedrun && totalSeconds <= 5 && totalSeconds > 0;
                ...
                <div className="flex items-center gap-3">
                    <div className="flex flex-col items-end justify-center">
                        ...
                        <Badge variant="secondary" className="text-xl font-bold px-3 py-1 mt-0.5">
                            {isSpeedrun && targetQuestions ? `${score}/${targetQuestions}` : score}
                        </Badge>
                    </div>
